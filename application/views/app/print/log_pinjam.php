...
<html lang="en">
  ...
  <img src="<?= base_url('assets/image/') ?>/Kayuh_Baimbai.png" width="20%" height="20%" align="left"/>
            ...
            <tr>
              <td>
                <?php if($filter['filter_awal'] != null && $filter['filter_akhir'] != null): ?>
                  Filter : 
                  <?= date('d M Y', strtotime($filter['filter_awal'])) . " - " . date('d M Y', strtotime($filter['filter_akhir'])) ?> / Rentang Tanggal / Log Buku Pinjam
                <?php endif; ?>
              </td>
              <td></td>
            </tr>
      ...
      <h3 class="text-center mb-2">LAPORAN BUKU PINJAM</h3>
