...
<html lang="en">
  <head>
    ...
    <title>Peminjaman Buku Populer</title>
  </head>
  <body>
    ...
    <div class="container my-5 py-5">
      ...
      <h3 class="text-center my-3">LAPORAN PEMINJAMAN BUKU POPULER</h3>
        ...
        <tbody>
          ...
          <?php foreach($all_laporan as $laporan): ?>
            <tr>
              ...
              <td><?= $laporan['judul'] ?></td>
              <td>
                <img src="<?= base_url('uploads/') . $laporan['gambar'] ?>" alt="tidak ada gambar" style="width: 100px">
              </td>
              <td><?= $laporan['penulis'] ?></td>
              <td><?= $laporan['penerbit'] ?></td>
              <td>
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=<?= $laporan['kode_buku'] . ';' . $laporan['ISBN'] . ';' . $laporan['judul'] . ';' . $laporan['penulis'] . ';' . $laporan['penerbit'] . ';'   ?>" alt="BUKU QR">  
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
