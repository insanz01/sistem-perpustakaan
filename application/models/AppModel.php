...
class AppModel extends CI_Model {
  ...
  public function total_visitor_today() {
    $query = "SELECT * FROM bukutamu WHERE DATE(created_at) = CURDATE()";
    $visitors = $this->db->query($query)->result_array();

    $queryMember = "SELECT * FROM bukutamu_membership WHERE DATE(created_at) = CURDATE()";
  ...
  public function total_visitor_week() {
    $query = "SELECT * FROM bukutamu WHERE WEEK(created_at) = WEEK(CURDATE())";
    $visitors = $this->db->query($query)->result_array();

    $queryMember = "SELECT * FROM bukutamu_membership WHERE WEEK(created_at) = WEEK(CURDATE())";
    $member_visitors = $this->db->query($query)->result_array();
